...
fn parse(input: &str, need_starts: bool) -> (&[u8], Vec<Node>, NodeMap) {
    ...
    for i in 0..n_lines {
        ...
        // Cycles are shorter when started from a Z node
        if need_starts && is_end(here) {
            starts.push(here);
        }
...
fn walk<'a>(directions: &'a [u8], map: &'a [[Node; 2]], mut here: Node) -> i64 {
    for (steps, _d) in directions.iter().cycle().enumerate() {
        here = map[here as usize][1];
...
// Part2 ========================================================================
#[aoc(day8, part2, cheese)]
pub fn part2(input: &str) -> i64 {
    let (directions, starts, map) = parse(input, true);

    assert!(!starts.is_empty());
    info!("directions.len()={}", directions.len());
